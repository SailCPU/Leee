...
Leee项目采用header-only设计理念，优先选择header-only第三方库和实现方式，以简化依赖管理、提升编译效率和增强跨平台兼容性。本文档定义了header-only的设计原则、实施指南和最佳实践。

## 1. 设计原则

### 1.1 Header-Only 优先原则

**核心理念**: 优先选择header-only库，避免动态链接库的复杂性。
